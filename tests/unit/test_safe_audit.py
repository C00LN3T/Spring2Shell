...
class TestSafeEncodingAudit:
    def test_returns_list(self) -> None:
        ...
        with patch("spring2shell.audit.safe_audit.create_stealth_session", return_value=session), \
            ...
            result = safe_encoding_audit("http://t.example",
                                         endpoints=["http://t.example/api"])
...
class TestSafeDependencyAudit:
    ...
    def test_detects_spring_boot_indicator(self) -> None:
        ...
        session.get.return_value = _make_response(
            '{"spring-boot": "2.7.0", "log4j": "2.14.1"}'
        )
    ...
    def test_detects_exposed_actuator(self) -> None:
        ...
        def get_side(url, *args, **kwargs):
            ...
            if url == "http://t.example":
                return _make_response("ok", headers={
...
class TestSafeFullAudit:
    def test_returns_overall_risk(self) -> None:
        ...
        mock_encoding = []
        mock_log = {"risk": "low", "versions_detected": [], "evidence": [],
                    "checked_paths": [], "target": "http://t.example"}
        mock_log_findings = []
        mock_deps = {"target": "http://t.example", "leaks": []}
        mock_misconfig = {"target": "http://t.example", "issues": []}

        with patch("spring2shell.audit.safe_audit.safe_encoding_audit", return_value=mock_encoding), \
             patch("spring2shell.audit.log_audit.run_log_audit", return_value=mock_log_findings), \
             patch("spring2shell.audit.log_audit.safe_log_audit", return_value=mock_log), \
             patch("spring2shell.audit.safe_audit.safe_dependency_audit", return_value=mock_deps), \
    ...
    def test_high_risk_when_log_risk_high(self) -> None:
        ...
        mock_log = {"risk": "high", "versions_detected": ["2.14.1"],
                    "evidence": ["Vulnerable version found"],
                    "checked_paths": [], "target": "http://t.example"}

        with patch("spring2shell.audit.safe_audit.safe_encoding_audit", return_value=[]), \
             patch("spring2shell.audit.log_audit.run_log_audit", return_value=[]), \
             patch("spring2shell.audit.log_audit.safe_log_audit", return_value=mock_log), \
             patch("spring2shell.audit.safe_audit.safe_dependency_audit",
                   return_value={"target": "http://t.example", "leaks": []}), \
             patch("spring2shell.audit.safe_audit.safe_misconfig_audit",
    ...
    def test_schema_keys_present(self) -> None:
        ...
        with patch("spring2shell.audit.safe_audit.safe_encoding_audit", return_value=[]), \
             patch("spring2shell.audit.log_audit.run_log_audit", return_value=[]), \
             patch("spring2shell.audit.log_audit.safe_log_audit",
                   return_value={"risk": "low", "versions_detected": [],
        ...
        for key in ("schema_version", "schema_type", "target", "encoding",
                    "log_risk", "dependency_leakage", "misconfiguration", "strict_summary"):
