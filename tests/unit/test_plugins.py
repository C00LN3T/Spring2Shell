...
class TestCVE202555182Plugin:
    def test_check_returns_finding_on_reflection(self):
        ...
        with patch("spring2shell.plugins.cve_2025_55182.waf_engine") as mock_waf, \
             patch("random.randint", return_value=999999), \
             patch("spring2shell.utils.auth.rate_limit_acquire"), \
             patch("spring2shell.utils.auth.audit_log"):
...
class TestCVE202566478Plugin:
    def test_check_returns_finding_on_reflection(self):
        ...
        with patch("spring2shell.plugins.cve_2025_66478.waf_engine") as mock_waf, \
...
class TestCVE202222965Plugin:
    def test_check_returns_potential_on_status(self):
        ...
        with patch("spring2shell.utils.auth.rate_limit_acquire"), \
             patch("spring2shell.utils.auth.audit_log"):
            finding = plugin.check(session, "http://t.example", "http://t.example/api", {})
