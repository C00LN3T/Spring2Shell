...
from spring2shell.core.exploiter import async_direct_exploit, async_cve_specific_scan
...
class TestAsyncScanner(unittest.TestCase):
    ...
    def test_create_async_session(self, mock_get_proxy, mock_get_auth):
        ...
        async def run():
            ...
            self.assertEqual(getattr(session, "proxy_url"), "http://proxy.example")
            await session.close()
    ...
    def test_async_direct_exploit_no_findings(self, mock_get_plugins, mock_fingerprint, mock_endpoints):
